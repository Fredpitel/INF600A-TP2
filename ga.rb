...
def lister( les_cours )  
  ...
  resultat = cours_to_s(liste_cours, options)
...
def nb_credits( les_cours )
  resultat = (ARGV.empty? ? 0 : ARGV.map { |sigle| get_cours(sigle, les_cours).nb_credits }.reduce(:+))
             .to_s << "\n"
  ARGV.clear
  [les_cours, resultat]
end
...
def prealables( les_cours )
  ...
  resultat = formater_resultat(prealables)
...
def cours_to_s( liste_cours, options )
